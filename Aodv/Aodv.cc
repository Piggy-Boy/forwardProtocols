...
void Aodv::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
    ...
    // When the stochastic number is less than the forwarding probality, discard the packet
    /*
    double pbty = uniform(0,1);
    if(pbty < this->forwardingProbability)
        return;
    */
    if(!tFt(rreq)){
...
bool Aodv::tFt(const Ptr<Rreq>& rreq)
{
    if(rreq->getReputation())
        return true;
    return false;
}

double Aodv::staticGame(double probability)
{
    return 0.5;
}

double Aodv::EvoGame(double probability)
{
    return 0.5;
}
