...
class INET_API Aodv : public RoutingProtocolBase, public NetfilterBase::HookBase, public cListener
{
  protected:
    ...
    // every node has a forwarding probability
    double forwardingProbability = 0.5;     // initial pbty is 0.5
  ...
  protected:
    ...
    bool tFt(const Ptr<Rreq>& rreq);
    double staticGame(double probability);
    double EvoGame(double probability);
